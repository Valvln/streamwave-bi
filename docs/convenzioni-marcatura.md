...
| `docs/kpi_operators.md` (007a) | **errore** |
| `docs/kpi_measures.md` (007b) | **errore** |
| `docs/raccomandazione.md` (009) | **errore** |
...
> **Nota in loco — 2026-08-21, feature `007a`.** «Le due righe finali» della nota qui sopra sono quelle della `005` e della `006`, che erano le ultime quando la nota è stata scritta. Da questa data non lo sono più: sotto di esse c'è la riga di `docs/kpi_operators.md`. Il testo della nota non viene riscritto — era esatto quando è stato scritto, e riscriverlo cancellerebbe la traccia di che cosa la `006` aveva effettivamente trovato.
...
| 2026-08-21 | 007a | il sesto documento in severità stretta, `docs/kpi_operators.md`, senza artefatto proprio: le sue ancore risolvono tutte contro artefatti di feature precedenti, sul precedente strutturale di `docs/data_model.md`. Nessun artefatto nuovo entra nello spazio dei nomi unito, e la grammatica non cambia |
| 2026-08-22 | 007b | il settimo documento in severità stretta, `docs/kpi_measures.md`, e **due** artefatti nuovi nello spazio dei nomi unito: `reports/kpi_measures.json` (quinto), scritto da uno script deterministico, e `reports/kpi_engine_check.json` (sesto), **curato a mano e mai scritto da alcuno script**, che congela l'esito del confronto fra le misure e il motore DAX reale. Il secondo esiste perché il primo non può contenerlo: un artefatto deterministico per costruzione non può ospitare una lettura umana senza smettere di esserlo, e senza un'ancora quell'esito non sarebbe pubblicabile in severità stretta. Estesa l'esclusione strutturale delle sigle di decisione da `[RDFVA]` a `[RDEFVA]`, perché questa feature numera le proprie decisioni con la lettera `E` |
| 2026-08-29 | 009 | l'ottavo documento verificato e il settimo in severità stretta, `docs/raccomandazione.md`. **Nessun artefatto nuovo** entra nello spazio dei nomi unito e la grammatica non cambia: le sei voci che la feature aggiunge — l'operatore della condizione `C2`, il suo margine in due forme, e il verdetto congiunto — vivono dentro `reports/kpi_measures.json`, che era già il quinto artefatto. È il primo documento del progetto scritto per un lettore che non apre il repository, e la sua severità stretta serve a una cosa che gli altri non chiedevano: che nessun numero della pagina più citabile fuori contesto sia stato digitato a mano. Registrata qui anche una zona di presidio umano più larga del solito, dichiarata in §6 e ripetuta nel documento: la pagina argomenta su una **maggioranza** e su una **metà**, e le quantità espresse per frazione o ordinale in lettere restano invisibili al controllo |
...
**L'estensione dell'esclusione strutturale alle sigle `E`, feature `007b`.** La classe delle sigle di decisione era `[RDFVA]`, scritta quando nessuna feature usava la lettera `E`. La `007b` numera le proprie decisioni `E1`-`E9`, e senza l'estensione ogni loro citazione sarebbe stata segnalata come cifra priva di marcatore in un documento a severità stretta. L'estensione è la stessa operazione già ammessa per le altre cinque lettere e non allarga il criterio in alcun altro senso: resta una sigla di una lettera seguita da cifre, chiusa da un confine di parola. È registrata qui perché una esclusione non dichiarata è una esclusione che nessuno può contestare, che è la ragione per cui §6 esiste.

**Lo stesso vale per le sigle di decisione con sotto-identificativo**, scoperto scrivendo il documento della `007a`: `D9.1` è riconosciuta dall'esclusione strutturale fino a `D9`, e la parte dopo il punto resta come cifra priva di marcatore. La chiusura è la stessa — apici inversi — e la ragione per non allargare il criterio anche: un'esclusione che inghiotte qualunque cifra dopo un punto smetterebbe di distinguere una sigla da un numero decimale.
