...
**Aggiornata**: 2026-08-23 | **Stato**: `007b` conclusa — otto KPI pubblicati e verificati contro il motore reale, un difetto di tipizzazione trovato e corretto prima che raggiungesse `008a`; `008a` apribile
...
| `008a` | Dashboard: modello, pagine, misure a schermo | 5 | 007b | ⬜ **apribile** |
...
**Totale residuo escluso `009`**: ~15 ore di feature, più ~1,7 ore di debito testuale — **~16,7 ore**. `007b` esce con tutti gli otto KPI pubblicati e **verificati contro il motore reale** (`E9`), non solo calcolati da script. Il debito che lascia è tutto rinviato come issue, per la regola del 2026-08-22: nessuna ora di piano, un rischio aperto — vedi [Rischi aperti](#rischi-aperti).
...
**`#18` merita un'attenzione che le altre sette non richiedono, ed è dichiarata qui perché riguarda direttamente la prossima feature.** `mood_profile_overlap` è corretta nel confronto che `E9` ha eseguito — nessun filtro di categoria era attivo — ma **`008a` costruisce pagine**, ed è esattamente il contesto in cui un filtro di categoria diventa naturale. Se la misura viene esposta accanto a un filtro senza l'`ALL` mancante, il numero si restringe silenziosamente senza che nulla lo segnali. Chi apre `008a` deve leggere questa issue prima di mettere `BQ1-K3` su una pagina filtrabile.
...
| **dal 24 agosto** | giornate piene, ~6 h/giorno | `008a`, `008b`, `010`, più ~1,7 h di debito testuale |

Atterraggio stimato: **26-27 agosto**, con `009` escluso. Era 21-22 prima dell'ancoraggio a benchmark della `004`, poi 24-25 con la finestra non pianificata del 12-14 agosto, poi 23-24 dopo le chiusure sotto stima di `003` e `004`, poi di nuovo 24-25 con la materializzazione del modello, poi 26-27 il mattino del 21 agosto, poi 25-26 la sera stessa con la `007a` sotto stima. Restano **~16,7 ore**, cioè meno di tre giorni pieni. Lo sforamento della `007b` — ~2 ore, il primo del progetto dopo cinque feature sotto stima — sposta l'atterraggio di un giorno, dentro la scadenza dell'abbonamento del 26 ma non più con il margine che la sera del 21 aveva riaperto: la pubblicazione di prova, già dichiarata caduta per la catena delle dipendenze, resta caduta anche per la data.
...
**Il difetto di tipizzazione può ripresentarsi, e nessun presidio di questo repository se ne accorgerebbe.** `E9` della `007b` ha trovato che tre colonne di `dim_track` (`energy`, `valence`, `danceability`) erano state caricate nel `.pbix` col punto decimale letto come separatore delle migliaia — un `Change Type` a locale italiano su un file in notazione inglese — e ha corretto il modello prima del merge. Il `.pbix` **non è versionato**: `scripts/check_audit_coherence.py` confronta documenti e artefatti, non entra mai nel modello, e una nuova materializzazione — quella che `008a` esegue per costruire le pagine — potrebbe ripetere esattamente lo stesso errore di caricamento senza che alcun controllo automatico lo segnali. Registrato come issue [`#11`](https://github.com/Valvln/streamwave-bi/issues/11).
