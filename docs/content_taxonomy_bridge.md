...
La decisione di processo è `DA-1`, presa dalla regia il 2026-08-19 e trascritta in [`mood_assignment_criteria.md`](mood_assignment_criteria.md): **un LLM propone, una persona decide**, e nessuno script chiama mai il modello. L'invocazione è un passaggio umano non riproducibile il cui esito si congela in un artefatto versionato — lo stesso schema del benchmark della `004`, dove il passaggio irripetibile era la raccolta di una fonte esterna invece della proposta di un modello.
...
### La ricognizione della regia sul chore `criterio-mood-cf1` — 2026-08-21

*Trascritta qui il 2026-08-29 dal piano di lavoro, che non è un artefatto pubblicato. È la fonte a cui le note in loco di questa sezione e di [`mood_assignment_criteria.md`](mood_assignment_criteria.md) rimandano.*

Prima di consegnare il chore che chiudeva `CF-1`, la regia ha aperto la tabella congelata, e **`CF-1` non aveva la forma in cui era stato registrato**.

**Come si comportano i valori.** Il secondo segnale di criterio §5 — il formato episodico ha cadenza più alta di quello a durata libera — è applicato dalla proposta su **otto<!--#--> coppie film/serie**: `Dramas`<!--@catalogs.mood_categories--> 0,30<!--@MOOD.category.dramas.mood_danceability--> contro `TV Dramas`<!--@catalogs.mood_categories--> 0,45<!--@MOOD.category.tv_dramas.mood_danceability-->, `Documentaries`<!--@catalogs.mood_categories--> 0,05<!--@MOOD.category.documentaries.mood_danceability--> contro `Docuseries`<!--@catalogs.mood_categories--> 0,20<!--@MOOD.category.docuseries.mood_danceability-->, `Horror Movies`<!--@catalogs.mood_categories--> 0,20<!--@MOOD.category.horror_movies.mood_danceability--> contro `TV Horror`<!--@catalogs.mood_categories--> 0,35<!--@MOOD.category.tv_horror.mood_danceability-->, e altre cinque<!--#-->. Non è applicato su **tre<!--#-->** coppie, e sono le tre<!--#--> in cui il termine film sta già al centro: `Anime Features`, `Movies`, `International Movies`.

**Nessun artefatto conteneva una regola di precedenza, ed è questo il punto.** Le cinque<!--#--> etichette in conflitto stavano a 0,50<!--#--> — il valore della proposta, superato dal chore — perché, e perché la verifica **si era astenuta deliberatamente**. La risoluzione economica — dichiarare che §2 prevale e non muovere nulla — non sarebbe stata «ratificare una regola già applicata»: sarebbe stata **eleggere a regola l'esito di un'astensione**. Il chore ha perciò dovuto decidere la precedenza **senza vedere la tabella**, e confrontare solo dopo.

**Il difetto di criterio §2 è più largo di `CF-1` e contiene anche `CF-2`.** §2 enumerava tre<!--#--> tipi di etichetta — generica, geografica o linguistica, di formato — e li trattava come esclusivi ed esaustivi. Il catalogo ne ha almeno altri tre<!--#--> che nessuna delle tre<!--#--> voci copre: **di pubblico** (`Kids' TV`, `Children & Family Movies`, `Teen TV Shows`), **di epoca o di ricezione** (`Classic Movies`, `Cult Movies`, `Classic & Cult TV`), **di modo di produzione o di tema** (`Independent Movies`, `LGBTQ Movies`). E `Korean TV Shows` mostra che i tipi non sono nemmeno esclusivi: è insieme geografica e di formato.

**Il ritrovamento puntuale su `Teen TV Shows`.** Stava a 0,50<!--#--> su tutti e tre<!--#--> gli assi — oggi l'asse del ritmo porta 0,55<!--@MOOD.category.teen_tv_shows.mood_danceability-->. È episodica a durata fissa e non è generica, geografica né linguistica: è **esattamente il caso** in cui la verifica aveva mosso `Anime Series` e `Classic & Cult TV`, con la motivazione che ha scritto due<!--#--> volte. Applicando quella stessa motivazione, la cella andava mossa e non lo era stata. È un errore di copertura della verifica, non una decisione — ed è la ragione per cui `MOOD.review.changes_count` è passato da 2<!--#--> a 3<!--@MOOD.review.changes_count-->.

**Esito, la sera del 2026-08-21.** Il subagent isolato ha ricevuto il solo criterio e ha risposto che i tre<!--#--> tipi di §2 non sono né esaustivi né esclusivi; la regola scritta in nota è che **fra un tipo che porta segnale e uno che non ne porta prevale il primo**, perché l'assenza di segnale non ha nulla da opporre a un segnale reale. Le cinque<!--#--> celle di `CF-1` si sono mosse da 0,50<!--#--> a 0,55<!--#--> sull'asse del ritmo, `Teen TV Shows` con esse ma per causa diversa. Una seconda verifica isolata ha controllato le sei<!--#--> celle contro il criterio v2<!--#--> confermandone la direzione e dichiarando che **il criterio autorizza la direzione, non la cifra**: 0,55<!--#--> resta un'assegnazione dell'analista dentro il verso consentito.

**La previsione della ricognizione era sbagliata sul merito, e va detto.** La regia aveva scritto che la risoluzione economica — §2 prevale, nulla si muove — era la trappola da cui guardarsi. La decisione presa al buio è andata nella direzione opposta, e sei<!--#--> celle si sono mosse. La cautela procedurale era giusta e l'esito che temeva non si è verificato: è il caso in cui un presidio funziona e non si vede.
