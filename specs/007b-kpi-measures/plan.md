...
**Storage**: `reports/kpi_measures.json` (nuovo, **quinto** artefatto in `ARTIFACTS` — oggi `(PROFILE, CLEANING, SCENARIOS, MOOD)`, quattro membri). `docs/kpi_measures.md` (nuovo, **settimo** documento in `DOCUMENTS` — oggi sei membri, di cui `docs/data_audit.md` unico a severità non stretta — quindi **sesto in severità stretta**). Nessun nuovo artefatto sotto `data/`: lo script legge, non congela, alcun dato assegnato a mano

**Testing**: verifica per esecuzione e per ispezione secondo [quickstart.md](./quickstart.md) — determinismo dello script (due esecuzioni, diff vuoto), presenza e ancoraggio di ciascuno degli otto valori, l'invarianza del numeratore della North Star dichiarata come verificata o come ritrovamento, la quota di zeri e l'avvertimento sui 7 segmenti `is_high_zero_genre`, i tre vincoli di §12 e le issue `#7`/`#8` chiusi con riferimento esplicito, esito verde di `check_audit_coherence.py` su sette documenti e cinque artefatti, la nota in loco su `business_case.md` §3, l'esito di E9 dichiarato per ciascuna misura, README allineato
...
**Scala/Ambito**: 8 KPI, di cui 3 pubblicano un valore per ciascuno dei 114 segmenti (`BQ2-K1`, `BQ2-K2`, `BQ2-K3`), 1 pubblica un conteggio per ciascuna delle 42 categorie come operatore di supporto (`BQ1-K1`/C1), 2 sono citazioni senza ricalcolo (`BQ3-K1`, `BQ3-K2`); 1 script nuovo; 1 artefatto JSON nuovo; 1 documento pubblicato nuovo; 2 decisioni nuove in `kpi_operators.md` (D10, D11); 2 correzioni "nota in loco" a documenti già mergiati; 1 estensione di `check_audit_coherence.py` (due righe: una in `ARTIFACTS`, una in `DOCUMENTS`); 2 righe nuove in `convenzioni-marcatura.md`; 1 passo manuale non scriptabile (E9, 8 confronti)
...
| **II. Riproducibilità** | trasformazioni come codice versionato | ✅ script deterministico sullo schema di `build_bq3_scenarios.py`, legge solo file già versionati o rigenerabili da `scripts/build_datasets.py` | ✅ US1 lo rende verificabile end-to-end: `data/raw/` → `build_datasets.py` → `build_kpi_measures.py`, non solo il secondo passo isolato |
...
reports/
└── kpi_measures.json               # NUOVO — quinto artefatto nello spazio dei nomi unito
...
| **F** | `docs/kpi_measures.md` aggiornato: ciascuna delle otto misure dichiara l'esito reale di ★ — «verificato contro il motore reale» o nota in loco con i due numeri e la causa | dopo ★ | il documento raggiunge la propria forma finale, pre-revisione |
...
| **H** | correzioni strettamente necessarie ai rilievi del verbale; issue non necessarie rinviate con numero; proposta di chiusura `#7`/`#8`; README allineato; riporto finale | dopo G | feature conclusa |
...
| F | 0,2 | incorporare l'esito di ★ nel documento, per ciascuna delle otto misure |
