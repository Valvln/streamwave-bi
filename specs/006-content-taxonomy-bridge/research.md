...
> **Nota in loco — 2026-08-20, feature `006`, task T005.**
> **Precedente**: il blocco di codice qui sopra dichiara `SP.num.valence.max → 1,0000` e `SP.num.danceability.max → 1,0000`, e il capoverso ne deriva che «i tre assi occupano per intero l'intervallo `0-1` sul dato osservato».
> **Corretto**: `SP.num.valence.max` risolve a `0,9950` e `SP.num.danceability.max` a `0,9850`. Solo `energy` tocca `1,0000`. L'affermazione «per intero» è vera per un asse su tre.
> **Causa**: la Fase 0 ha trascritto i sei valori senza risolverli uno per uno contro l'artefatto, generalizzando dai due estremi di `energy`. È esattamente la categoria di errore che la regola sulle affermazioni derivate (`CLAUDE.md`) descrive: nessun valore conteneva «per intero», e nessun controllo la verificava.
> **Fonte**: `reports/data_profile.json`, sei identificativi risolti in T005. Il minimo è `0,0000` su tutti e tre gli assi; i massimi sono `1,0000` (energy), `0,9950` (valence), `0,9850` (danceability).
> **Conseguenza operativa**: nessuna per l'impianto — D2 vincola la *scala* del lato video a quella del lato musicale, cioè al dominio decimale `0-1` dichiarato da §11 di `docs/data_model.md`, non all'estensione empirica della distribuzione. Cambia però ciò che il criterio può scrivere: ancora i propri estremi ai sei identificativi **per il valore che risolvono**, e dichiara che su `valence` e `danceability` il massimo osservato sta poco sotto il limite superiore del dominio. Il testo del capoverso originale non si riscrive.
