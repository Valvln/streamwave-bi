...
3. **La verifica indipendente.** Chi non ha prodotto la proposta verifica ogni riga contro il criterio del passo 1, che è l'**unico metro** di contestazione ammesso — non un vincolo su che cosa il verificatore possa leggere. L'accesso a `reports/data_profile.json`, e in particolare ai sei identificativi di ancoraggio di FR-004, è necessario per verificare l'obbligo di scala di D2, e il verificatore lo ha. Produce la tabella rivista insieme al conteggio di quante righe ha spostato rispetto alla proposta. Non è la revisione in contesto pulito di `CLAUDE.md`: è un passo di lavorazione distinto, chiuso da D9.
...
Chi verifica non è la persona (o la sessione) che ha ottenuto la proposta dal modello. Riceve la proposta e il criterio e ha accesso al resto del repository — in particolare a `reports/data_profile.json`, per verificare gli ancoraggi di FR-004 — perché il criterio è il **metro** di contestazione, non un perimetro di lettura: controlla ogni riga contro il criterio, e solo contro quello. Il numero di righe spostate rispetto alla proposta si scrive nell'artefatto congelato, non solo in prosa altrove.
...
- **FR-008**: La proposta MUST essere verificata da chi non l'ha prodotta, contro il criterio (FR-001) come **unico metro di contestazione** — non contro il modello dati, non contro le feature precedenti, non contro un'opinione indipendente sul mood di una categoria. Non è un vincolo su **che cosa** il verificatore possa leggere: la verifica MUST poter accedere a `reports/data_profile.json`, e in particolare ai sei identificativi di ancoraggio richiesti da FR-004, perché senza di essi l'obbligo di scala di D2 — quello che la spec chiama «l'obbligo che conta di più», proprio perché un suo errore non produce alcun sintomo visibile — non è verificabile. Questo passo NON DEVE essere confuso con la revisione in contesto pulito di `CLAUDE.md` (FR-011a): è un passo di lavorazione interno alla costruzione della tabella, non richiede l'isolamento stretto da subagent — quell'isolamento serve a FR-011a, non qui — e avviene prima del congelamento, con pieno accesso al resto del repository.
