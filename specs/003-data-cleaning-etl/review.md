...
## Chiusura delle divergenze — 2026-08-15

*Secondo blocco aggiunto in coda, per decisione della regia. Il verbale resta non modificato. Le divergenze 6 e 8 erano già chiuse nel blocco precedente.*

**Divergenza 7 — chiusa, con lavoro.** La definizione normativa della grammatica di marcatura è stata trasferita in [`docs/convenzioni-marcatura.md`](../../docs/convenzioni-marcatura.md), che unisce §4 del contratto della 002 e §3 di quello della 003. §2 del documento pubblicato, il commento di intestazione dello script e il README rinviano lì; i due contratti portano una nota di trasferimento datata che dichiara quale testo prevale in caso di divergenza, senza che nulla sia stato rimosso. La ragione per farlo dentro questa feature e non dopo non è il lettore esterno: è che la 004 scriverà prosa con la stessa grammatica e avrebbe dovuto citare la cartella di lavorazione di un'altra feature. Da lì in poi ogni documento avrebbe ereditato il rinvio sbagliato.

**Divergenza 2 — chiusa: un solo marcatore.** La distinzione fra *non misurabile* e *misurato altrove* non diventa una quinta forma. I tre casi che il verbale portava sono già risolti in modi diversi e coerenti fra loro: il 2021 è stato ancorato, perché un valore ancorabile si ancora; l'aneddoto dell'invariante è stato riformulato, perché la prosa deve dire «non verificabile» dove il marcatore non lo distingue da un numerale retorico; il conteggio degli avvisi è stato rimosso. La regola è ora scritta in §2 del file consolidato, con le quattro categorie che il marcatore copre. L'onere di distinguerle resta sulla prosa, dove un lettore le coglie meglio che da un simbolo in più.

**Divergenza 3 — chiusa ratificando la forma attuale.** L'elenco normativo delle esclusioni resta dentro `scripts/check_audit_coherence.py`, e questa è la forma definitiva e non un ripiego: una esclusione vive dove viene applicata, altrimenti le due copie divergono e nessuna delle due è vera. Il documento pubblicato ne nomina le classi e rinvia allo script per la lista — è ciò che la correzione di R3 ha già fatto, e §6 del file consolidato lo dichiara come regola invece di lasciarlo come pratica.

**Divergenza 4 — chiusa: le due regole convivono, e la convivenza è dichiarata.** Il corollario (b) di D5 vieta i numerali in lettere per i fatti misurati; la seconda forma della grammatica resta accettata dal controllo perché la usa il documento della 002. La forma **non** viene rimossa: rimuoverla romperebbe un artefatto già mergiato per guadagnare una semplificazione che nessuno sta chiedendo. È la precisazione che accompagna D5 in `CLAUDE.md`, ed è ora scritta in §7 del file consolidato.

**Le divergenze 1 e 5 restano aperte, per scelta.** La 1 — la precisione a cui confrontare profilo e rendiconto — cambierebbe numeri pubblicati, obbligherebbe a rieseguire la pipeline e a decidere se il profilo della 002 abbia un difetto da registrare come ritrovamento. Non è urgente: il documento pubblica ora i due valori che misurano quanta parte del divario sia apparente, quindi nessun lettore è indotto in errore. La 5 — se gli attributi dei tre titoli riparati debbano entrare nel rendiconto — è una decisione sul perimetro degli artefatti, non sulla singola frase: oggi il rendiconto contiene aggregati e identificativi, e ammettere attributi di record individuali si decide una volta per tutte. Entrambe vanno al debito della roadmap.
