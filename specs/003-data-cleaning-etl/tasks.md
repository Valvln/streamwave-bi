...
- [ ] T008 [US1] Implementare in `scripts/build_datasets.py` la separazione del campo `duration` in `movie_duration_min` e `tvshow_seasons` (FR-014), con l'invariante di F5: ogni `Movie` porta minuti, ogni `TV Show` porta stagioni, nessuna forma terza. È una **verifica**, non una deduzione dalla stringa: se fallisce, la fonte è cambiata e la pipeline si ferma
- [ ] T009 [US1] Implementare in `scripts/build_datasets.py` la riparazione dello scivolamento di colonna della decisione ereditata **D2**: spostare nel campo durata il valore del campo di classificazione che soddisfa la forma dichiarata, su righe con durata vuota; porre a mancante la classificazione di quelle righe; marcarle con `is_repaired_duration`. Il raggio d'azione atteso è di **tre** righe (F1) e la pipeline si ferma se ne tocca un numero diverso (FR-016, FR-004)
- [ ] T010 [US1] Implementare in `scripts/build_datasets.py` il controllo di dominio sul campo di classificazione contro `conventions.rating_domain` del profilo: i valori fuori dominio residui vanno posti a mancante e contati, mai indovinati (FR-015). Il controllo di dominio e la riparazione di T009 sono **due operazioni distinte**, e la seconda non è la conseguenza automatica della prima
- [ ] T011 [US1] Implementare in `scripts/build_datasets.py` la conversione di `date_added` in ISO 8601 usando la mappa dei mesi di T003 e normalizzando lo spazio iniziale degli 88 valori di F6. **Vietato** `strptime` con `%B`, che dipende dal locale e produrrebbe risultati diversi su macchine diverse (T6, FR-003)
